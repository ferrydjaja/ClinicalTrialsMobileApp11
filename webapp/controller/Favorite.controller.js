    ...
    return Controller.extend("ClinicalTrials.ClinicalTrials.controller.Favorite", {
        ...
        onProcess: function() {
            ...
            if (oStorage.get('NCTID') == null) {
                ...
                $.ajax({
                    ...
                    async: true,
                    cache: true,
                    url: conn + "?q=6&nctnumbers=" + oStorage.get('NCTID'),
                    timeout: 600000,
                    dataType: "json",
        ...
        handleOnPress: function(oEvent) {
			...
			this.onCheckCert(function(returnValue) {
				console.log(returnValue);
				if(returnValue == 'success') {
					this_.onProcess2(oEvent, nct_id);
		...
		onProcess2: function(oEvent, nct_id) {
            var busyDialog = (busyDialog) ? busyDialog : new sap.m.BusyDialog({
                text: "{i18n>MSG0}",
                title: "{i18n>MSG1}"
            });
        ...
        deleteNCTOnPress_: function(oEvent) {  
			jQuery.sap.require("jquery.sap.storage");
            var oStorage = jQuery.sap.storage(jQuery.sap.storage.Type.local);
		...
		deleteNCTOnPress: function(oEvent) {            
			...
			this.onCheckCert(function(returnValue) {
				...
				if(returnValue == 'success') {
					...
					this_.deleteNCTOnPress_(oEvent);
